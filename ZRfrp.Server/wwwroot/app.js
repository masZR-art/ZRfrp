...
function showApp(role){document.body.dataset.role=role;$("#login").classList.add("hidden");$("#app").classList.remove("hidden");$$(".admin-only").forEach(x=>x.classList.toggle("hidden",role!=="admin"));$$(".customer-only").forEach(x=>x.classList.toggle("hidden",role!=="customer"));openPage(role==="admin"?"overview":"customer")}
...
function askConfirmation(title,message){$("#confirm-title").textContent=title;$("#confirm-message").textContent=message;$("#confirm-dialog").classList.remove("hidden");return new Promise(resolve=>{confirmResolver=resolve})}
...
async function loadCustomer(){try{const me=await api("/api/customer/me");$("#customer-used").textContent=fmtBytes(me.trafficUsedBytes);$("#customer-quota").textContent=me.trafficQuotaBytes?fmtBytes(me.trafficQuotaBytes):"不限";$("#customer-remaining").textContent=fmtBytes(me.remainingBytes);$("#customer-info").innerHTML=`<div class="detail"><span>账号</span><strong>${escapeHtml(me.username)}</strong></div><div class="detail"><span>账号 ID</span><strong>${escapeHtml(me.id)}</strong></div><div class="detail"><span>绑定邮箱</span><strong>${escapeHtml(me.maskedEmail||"未绑定")}</strong></div>`;$("#customer-email-hint").textContent=me.hasEmail?`验证码将发送至 ${me.maskedEmail}`:"当前账号未绑定邮箱，请联系管理员处理";$("#send-customer-reset-code").disabled=!me.hasEmail;$("#node-health").className="health online";$("#node-health").innerHTML="<i></i>账号正常";await loadTrafficDashboard("customer")}catch(e){toast(e.message)}}
